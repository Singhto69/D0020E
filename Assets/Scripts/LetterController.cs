...
public class LetterController : MonoBehaviour
{
    ...
    static Dictionary<string, int> rowPos = new Dictionary<string, int>()
        {
            { "D", 14019 }, { "O", 68796 }, { "R", 78930 }, { "�", 119847 }, { "S", 84112 }, { "�", 118939 },
            { "E", 17823 }, { "T", 102186 }, { "L", 54825 }, { "A", 0 }, { "F", 20401 }, { "�", 120464 },
    ...
    {
        foreach (var line in System.IO.File.ReadLines(@"Assets\ordlista.txt")) // �ndra k�lla
        {
            dictionary.Add(line);
            size++;
        }
    ...
    public static void searchWord()
    {
        ...
        for (int numLetters = tempNumLetters; numLetters <= wordLength; numLetters++)
        {
            ...
            wordPosition = rowPos[subWordFirstLetter.ToUpper()]; // hitta position var ord ska s�kas
            bool exist = false;
            ...
            for (int arrayPosition = tempWordPosition; arrayPosition < size; arrayPosition++)
            {
                ...
                {
                    string subWord = dictionary[arrayPosition].ToLower().Substring(0, numLetters); // f�rsta delen av ordet
                    if (!word.Substring(0, 1).Equals(subWordString.Substring(0, 1)) || arrayPosition.Equals(size - 1)) // har vi s�kt igenom alla ord som b�rjar p� bokstav ?
                    {
                        if (!exist) // hittas inte ordet s� k�r funktionen igen med en bokstav mindre
                        {
                            LetterController.slicedText.text = wordString.Substring(1, wordLength - 1).ToUpper(); // ta bort f�rst bokstaven fr�n str�ngen
                            tempNumLetters = 1;
                    ...
                    if (subWordString.Equals(subWord)) // kolla om b�rjan p� ordet existerar. spara index f�r att kunna forts�tta d�rifr�n senare
                    {
